...
const RecommendationScreen = ({ navigation }) => {
  ...
  const { user, tags } = useUserStore(); // ✅ tags 가져오기
  const [tagScores, setTagScores] = useState({});

  // ✅ 초기화: 로그인 후 받아온 tags를 tagScores에 반영
  useEffect(() => {
    if (tags) {
      setTagScores({
        spicy: tags.spicy, 
        value_for_money: tags.value_for_money,
        kindness: tags.kindness,
        cleanliness: tags.cleanliness,
        atmosphere: tags.atmosphere,
        large_portions: tags.large_portions,
        tasty: tags.tasty,
        waiting: tags.waiting,
        sweet: tags.sweet,
        salty: tags.salty,
        savory: tags.savory,
        freshness: tags.freshness,
        solo_dining: tags.solo_dining,
        trendy: tags.trendy,
